...
if (size.includes('GB') || size.replace(' MB', '') > 2500) { return await m.reply('El archivo pesa mas de 2500 MB, se canceló la Descarga.')}
let txt = `*乂  A P T O I D E  - DESCARGA*\n\n`
    txt += `	 🝪 *Nombre* : ${name}\n`
    txt += `	 🝪 *Package* : ${packname}\n`
    txt += `	 🝪 *Update* : ${update}\n`
    txt += ` 	🝪 *Peso* :  ${size}\n\n`
    txt += `*- ↻ El archivo se esta enviando espera un momento 🌀░S░o░n░G░o░k░u🌀. . .*`
